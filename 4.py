...
with open("scientist.txt", encoding="utf-8") as f:
    ...
    for i in s:
        ...
        i["password"] = random.choices(ascii_letters, k=10) + random.choices(digits, k=10)
        random.shuffle(i["password"])
        i["password"] = "".join(i["password"])


with open("scientist_password.csv", "w", encoding="utf-8", newline="") as f:
    writer = csv.DictWriter(f, fieldnames=["ScientistName","preparation", "date", "components", "login", "password"], delimiter = "#")
    writer.writeheader()
    writer.writerows(s)