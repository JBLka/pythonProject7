...
def quicksort(n):
    if len(n) <= 1:
        ...
        q = datetime.date(list(map(int, x["date"].split("-")))[0], list(map(int, x["date"].split("-")))[1],
                          list(map(int, x["date"].split("-")))[2])
        for z in n:
            b = datetime.date(list(map(int, z["date"].split("-")))[0], list(map(int, z["date"].split("-")))[1],
                              list(map(int, z["date"].split("-")))[2])
            if b < q:
                l_list.append(z)
            elif b > q:
                r_list.append(z)
